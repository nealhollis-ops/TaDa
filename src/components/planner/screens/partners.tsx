...
import { CheckCircle2, Circle, RefreshCw, Send, X } from "lucide-react";
...
export function PartnersScreen() {
  ...
  return (
    <div className="px-5 py-5">
        ...
        <div className="mb-3">
          ...
          {p.incoming.map((r) => (
            <div key={r.id} className="mb-2 flex items-center gap-2 rounded-2xl p-3" style={{ background: "#fff", border: `1px solid ${C.gold}` }}>
              ...
              {!canAddPartner ? (
                <Link href="/account" className="rounded-xl px-3 py-1.5 text-xs font-semibold" style={{ background: "#EDF3F2", color: C.fade }}>
                  Upgrade to add
                </Link>
              ) : (
              ...
              <button onClick={() => void p.declineRequest(r)} className="rounded-xl px-2 py-1.5" style={{ background: "#EDF3F2" }} aria-label="Decline">
        ...
        seekers.map((s) => {
          ...
          return (
            <div key={s.id} className="mb-2 flex items-center gap-2 rounded-2xl p-3" style={{ background: "#fff" }}>
              ...
              <button onClick={() => void p.sendRequest(s.id)} disabled={disabled} className="rounded-xl px-3 py-1.5 text-xs font-semibold" style={{ background: disabled ? "#EDF3F2" : C.navy, color: disabled ? C.fade : C.cream }}>
      ...
      <h3 className="mb-2 mt-5 text-sm font-bold" style={{ color: C.navy }}>
        Teams
      </h3>
      {p.plan === "standard" ? (
        <div className="mb-3 rounded-2xl p-4" style={{ background: C.navy }}>
        ...
        <div>
          {p.myInvites.map((inv) => (
            <div key={inv.id} className="mb-2 flex items-center gap-2 rounded-2xl p-3" style={{ background: "#fff", border: `1px solid ${C.gold}` }}>
              ...
              <button onClick={() => void p.answerInvite(inv, false)} className="rounded-xl px-2 py-1.5" style={{ background: "#EDF3F2" }} aria-label="Decline">
                <X size={14} style={{ color: C.fade }} />
              </button>
            </div>
          ))}
          ...
          <div className="mb-3 flex gap-2 rounded-2xl p-3" style={{ background: "#fff" }}>
            <input
              ...
              style={inputStyle}
              placeholder={p.plan === "boss" ? "Name a new boss team..." : "Name a new team..."}
              value={teamName}
      ...
      <p className="mt-3 text-xs" style={{ color: C.fade }}>
        Messages arrive live. If something looks stale, tap the refresh arrow.
      </p>
    </div>
  );
}
...
function TeamCard({ t }: { t: Team }) {
  ...
  const visibleAssignments = p.assignments.filter((a) => a.teamId === t.id && a.toUser && (owner || a.toUser === p.me.id));
  ...
  return (
    <div className="mb-3 rounded-2xl p-4" style={{ background: "#fff", border: `2px solid ${C.gold}` }}>
      <button onClick={() => p.set("openTeam", open ? null : t.id)} className="flex w-full items-center justify-between">
        <span className="text-sm font-bold" style={{ color: C.navy }}>
          {t.name}
        </span>
        <span className="text-xs" style={{ color: C.fade }}>
          {t.members.length} of {TEAM_CAP}
        </span>
      </button>
      {open && (
        <div className="mt-3">
          ...
          {t.kind === "boss" && (
            <div className="mt-3">
              <div className="mb-2 text-xs font-semibold" style={{ color: C.coral }}>
                Assigned work
              </div>
              {owner && (
                <div className="mb-2 rounded-xl p-3" style={{ background: C.cream }}>
                  ...
                  <div className="flex gap-2">
                    <select className="flex-1 rounded-xl border px-2 py-2 text-sm" style={inputStyle} value={assignDay} onChange={(e) => setAssignDay(e.target.value)}>
              ...
              {owner && tank.length > 0 && (
                <div className="mb-2 rounded-xl p-3" style={{ background: "#FFE2DB", border: `1px solid ${C.coral}` }}>
              ...
              {visibleAssignments.map((a) => {
                ...
                return (
                  <div key={a.id} className="mb-1.5 flex items-center gap-2 rounded-xl px-3 py-2" style={{ background: C.cream, opacity: a.done ? 0.65 : 1 }}>
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="mt-0.5 flex gap-1" style={{ fontSize: 10, color: late ? C.coral : C.fade }}>
                        <span>{p.nameOf(a.toUser)}</span>
                        <span>·</span>
                        <span>{a.date ? (late ? `Overdue, was ${dayLabel(p.month, a.date)}` : `Due ${dayLabel(p.month, a.date)}`) : "No deadline"}</span>
                      </div>
                    </div>
                    {owner && (
                      <button onClick={() => void p.removeAssigned(a.id)} className="shrink-0" aria-label="Remove">
