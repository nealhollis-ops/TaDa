...
export async function POST(request: Request) {
  ...
  const { error } = await resend.emails.send({
    ...
    html: `<!doctype html><html><body style="margin:0;padding:0;background:#F0F9FF;font-family:-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;color:#163B52;">
...
<p style="margin:0 0 20px;font-size:14px;line-height:1.5;color:#4B6B82;">${kindLine}</p>
<p style="margin:0 0 24px;text-align:center;"><a href="${link}" style="display:inline-block;background:#F43F5E;color:#ffffff;text-decoration:none;font-weight:700;font-size:16px;padding:14px 28px;border-radius:999px;">Join the team</a></p>
<p style="margin:0 0 8px;font-size:13px;line-height:1.5;color:#4B6B82;">Or copy this link: <span style="word-break:break-all;color:#0C4A6E;">${link}</span><br>This invitation expires in 14 days.</p></td></tr>
<tr><td style="padding:16px 32px 28px;font-size:12px;line-height:1.5;color:#4B6B82;border-top:1px solid #BFE3F7;">Not expecting this? You can ignore it. Questions? Write to clientcare@gettada.me.</td></tr>
